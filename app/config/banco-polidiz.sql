...
CREATE TABLE IF NOT EXISTS `polidez`.`Propostas` (
  `idProjetosEleito` INT NOT NULL,
  `tituloProjEleito` VARCHAR(45) NULL DEFAULT NULL,
  `statusProjEleito` VARCHAR(45) NULL DEFAULT NULL,
  `dataProjEleito` VARCHAR(45) NULL DEFAULT NULL,
  `descricaoProjEleito` VARCHAR(45) NULL DEFAULT NULL,
  `Politicos_idPoliticos` INT NOT NULL,
  PRIMARY KEY (`idProjetosEleito`),
  UNIQUE INDEX `idProjetosEleito_UNIQUE` (`idProjetosEleito` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `polidez`.`resposta` (
  ...
  `id_respondente` INT NULL,
  `tipo_respondente` INT NULL,
  `resposta` VARCHAR(45) NULL,
  `data_resposta` VARCHAR(45) NULL,
  `mensagem_idmensagem` INT NOT NULL,
  PRIMARY KEY (`idresposta`),
  INDEX `fk_resposta_mensagem1_idx` (`mensagem_idmensagem` ASC) VISIBLE,
  CONSTRAINT `fk_resposta_mensagem1`
    FOREIGN KEY (`mensagem_idmensagem`)
    REFERENCES `polidez`.`mensagem` (`idmensagem`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
